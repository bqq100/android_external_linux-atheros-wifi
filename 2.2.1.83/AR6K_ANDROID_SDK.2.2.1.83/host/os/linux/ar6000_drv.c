 ...
 * WLAN Device
 */
static const char athId[] __attribute__ ((unused)) = "$Id: //depot/sw/releases/olca2.2/host/os/linux/ar6000_drv.c#48 $";
...
int wowenable = 0; /* 1 to enable WoW automatically */
...
static void ar6000_stop_endpoint(struct net_device *dev);
...
static A_STATUS ar6000_suspend_ev(void *context)
{
    ...
    if (wowenable && ar->arWmiReady && ar->arConnected) {
        ar->arOsPowerCtrl = WLAN_PWR_CTRL_WOW;
        /* leave for pm_device to setup wow */
        return -1; /* return -1 will remain power for host controller */
    } else {
        ar->arOsPowerCtrl = WLAN_PWR_CTRL_CUT_PWR;
        ar->arResumeConnection = ar->arConnected;
        ar6000_stop_endpoint(ar->arNetDev);
    }
    return A_OK;
}
...
static A_STATUS ar6000_resume_ev(void *context)
{
    ...
    if (powerCtrl==WLAN_PWR_CTRL_WOW) {
...
ar6000_restart_endpoint(struct net_device *dev)
{
    ...
    do {
        ...
        if (ar6000_download_image(ar->arNetDev)!=A_OK)
            break;
        ar->arWlanState = WLAN_ENABLED;
        if (ar->arResumeConnection && ar->arSsidLen) {
            if (ar6000_connect_to_ap(ar) != A_OK)
            {
                ar->arResumeConnection = FALSE;
            } 
        }
        return;
    } while (0);
...
static void
ar6000_stop_endpoint(struct net_device *dev)
{
    AR_SOFTC_T *ar = (AR_SOFTC_T*)netdev_priv(dev);
    printk("%s -- enter\n", __func__);
        ...
        if (!bypasswmi)
        {
            if (ar->arConnected == TRUE || ar->arConnectPending == TRUE)
            {
                if (ar->arResumeConnection) {
                    ...
                    /* wait for disconnect cmd by invoke another get bitrate cmd*/
                    if (ar->arTxPending[ar->arControlEp]) {
                        long timeleft = wait_event_interruptible_timeout(arEvent,
                                        ar->arTxPending[ar->arControlEp] == 0, wmitimeout * HZ);
                        if (!timeleft || signal_pending(current)) {
                        ...
                        A_UINT8 assocRespLen, A_UINT8 *assocInfo, A_UINT16 protocolReasonStatus)
{
    ...
    if ((TRUE == ar->arConnected || ar->arResumeConnection) && (reason == NO_NETWORK_AVAIL))
    {
